...
// === DASHBOARD CONNECTOR ===
// State is held in a plain DcState struct; all operations are DC_-prefixed free functions.
struct DcState {
   string url;
   string psk;
   ...
   bool   debug_log;
   ulong  last_log_ms;
   uint   last_live_payload_hash;
   long   last_live_payload_sent_ms;
   string last_ack_history_hash;
   bool   startup_health_checked;
   bool   last_sync_ok;
   long   history_start_time_ms;
};
DcState g_dc;

uint DC_HashPayload(const string payload) {
   uchar bytes[];
   int n = StringToCharArray(payload, bytes, 0, WHOLE_ARRAY, CP_UTF8);
   if(n <= 1) return 0;
   uint hash = 2166136261;
   for(int i = 0; i < n - 1; i++) { hash ^= (uint)bytes[i]; hash *= 16777619; }
   return hash;
}

string DC_ExtractJsonString(const string json, const string key) {
   string token = "\"" + key + "\":\"";
...
bool DC_ExtractJsonBool(const string json, const string key, bool default_value = false) {
   if(StringFind(json, "\"" + key + "\":true")  >= 0) return true;
   if(StringFind(json, "\"" + key + "\":false") >= 0) return false;
   return default_value;
}

string DC_ToHex(const uchar &bytes[]) {
   string out = "";
   int n = ArraySize(bytes);
   for(int i = 0; i < n; i++) out += StringFormat("%02x", bytes[i]);
   return out;
}

bool DC_Sha256(const uchar &payload_bytes[], uchar &digest[]) {
   uchar no_key[];
   ArrayResize(no_key, 0, 0);
   return CryptEncode(CRYPT_HASH_SHA256, payload_bytes, no_key, digest) > 0;
}

bool DC_BuildHmacSha256(const uchar &key[], const uchar &message[], uchar &out_digest[]) {
   const int BLOCK_SIZE = 64;
   uchar normalized_key[];
   ArrayCopy(normalized_key, key);
   if(ArraySize(normalized_key) > BLOCK_SIZE) {
      uchar key_hash[];
      if(!DC_Sha256(normalized_key, key_hash)) return false;
   ...
   if(ArraySize(normalized_key) < BLOCK_SIZE) {
      ...
      for(int i = old_size; i < BLOCK_SIZE; i++) normalized_key[i] = 0;
   }
   uchar ipad[], opad[];
   ...
   for(int i = 0; i < BLOCK_SIZE; i++) inner_data[i] = ipad[i];
   for(int i = 0; i < message_size; i++) inner_data[BLOCK_SIZE + i] = message[i];
   uchar inner_hash[];
   if(!DC_Sha256(inner_data, inner_hash)) return false;
   uchar outer_data[];
   int inner_size = ArraySize(inner_hash);
   ArrayResize(outer_data, BLOCK_SIZE + inner_size, 0);
   for(int i = 0; i < BLOCK_SIZE; i++) outer_data[i] = opad[i];
   for(int i = 0; i < inner_size; i++) outer_data[BLOCK_SIZE + i] = inner_hash[i];
   return DC_Sha256(outer_data, out_digest);
}

string DC_CreateSignature(string account_number, long timestamp_ms) {
   string message = account_number + ":" + StringFormat("%lld", timestamp_ms);
   uchar message_bytes[], key_bytes[], digest[];
   int n = StringToCharArray(message, message_bytes, 0, WHOLE_ARRAY, CP_UTF8);
   if(n > 0) ArrayResize(message_bytes, n - 1, 0);
   int k = StringToCharArray(g_dc.psk, key_bytes, 0, WHOLE_ARRAY, CP_UTF8);
   if(k > 0) ArrayResize(key_bytes, k - 1, 0);
   if(!DC_BuildHmacSha256(key_bytes, message_bytes, digest)) {
      if(g_dc.debug_log) Print("[DC] Failed to build HMAC signature");
      return "";
   }
...
string DC_BuildClosedTradesJson(long &latest_closed_time_ms, string &latest_closed_deal_id) {
   string j = "[";
   bool first_trade = true;
   latest_closed_time_ms = 0;
   latest_closed_deal_id = "";
   long history_start_ms = g_dc.history_start_time_ms;
   datetime hist_select_start = (history_start_ms > 0) ? (datetime)(history_start_ms / 1000) : 0;
   if(!HistorySelect(hist_select_start, TimeCurrent())) return j + "]";
   ...
   for(int i = 0; i < deals_total; i++) {
      ...
      if(deal_symbol == "" || position_id <= 0) continue;
      ...
      for(int idx = 0; idx < entry_count; idx++) {
         if(entries[idx].active && entries[idx].position_id == position_id) { pos_idx = idx; break; }
      }
      if(deal_entry == DEAL_ENTRY_IN) {
         if(pos_idx >= 0) {
            double new_total = entries[pos_idx].open_volume + deal_volume;
            if(new_total > 0.0) {
               entries[pos_idx].avg_entry_price = (entries[pos_idx].avg_entry_price * entries[pos_idx].open_volume + deal_price * deal_volume) / new_total;
               entries[pos_idx].open_volume = new_total;
               if(deal_time_ms < entries[pos_idx].first_entry_time_ms) entries[pos_idx].first_entry_time_ms = deal_time_ms;
            }
      ...
      if(deal_entry == DEAL_ENTRY_OUT || deal_entry == DEAL_ENTRY_OUT_BY) {
         ...
         if(pos_idx >= 0) {
            ...
            if(entries[pos_idx].open_volume <= 0.00000001) { entries[pos_idx].open_volume = 0.0; entries[pos_idx].active = false; }
         }
...
void DC_PostSync(string payload, string account_number, string signature, long timestamp_ms,
                 uint live_payload_hash, bool include_history, string history_hash) {
   uchar req[], resp[];
   string resp_hdrs = "";
   string hdrs = "Content-Type: application/json\r\n"
                 "Authorization: HMAC-SHA256 " + signature + "\r\n"
                 "X-Signature-Timestamp: " + (string)timestamp_ms + "\r\n";
   int n = StringToCharArray(payload, req, 0, WHOLE_ARRAY, CP_UTF8);
   if(n > 0) ArrayResize(req, n - 1, 0);
   int res = WebRequest("POST", g_dc.url + "/api/ingestion", hdrs, 5000, req, resp, resp_hdrs);
   if(res == -1) {
      g_dc.error_count++;
      g_dc.last_sync_ok = false;
      if(g_dc.debug_log) { Print("[DC] WebRequest error: ", GetLastError()); ResetLastError(); }
   } else {
      ...
      if(res >= 200 && res < 300) {
         ...
         g_dc.success_count++;
         g_dc.last_sync_ok = true;
         g_dc.last_live_payload_hash = live_payload_hash;
         g_dc.last_live_payload_sent_ms = timestamp_ms;
         if(g_dc.debug_log)
            Print("[DC] Sync accepted (", StringLen(payload), " bytes, status=", res, ")");
      } else {
         g_dc.error_count++;
         g_dc.last_sync_ok = false;
         if(g_dc.debug_log) Print("[DC] Sync rejected (status=", res, ", body=", body, ")");
      }
   }
   g_dc.in_flight = false;
}
...
bool DC_PostHealthCheck(string account_number, long timestamp_ms, string history_hash,
   ...
   int n = StringToCharArray(payload, req, 0, WHOLE_ARRAY, CP_UTF8);
   if(n > 0) ArrayResize(req, n - 1, 0);
   int res = WebRequest("POST", g_dc.url + "/api/ingestion/health", hdrs, 5000, req, resp, resp_hdrs);
   if(res == -1) {
      if(g_dc.debug_log) { Print("[DC] Health check error: ", GetLastError()); ResetLastError(); }
      return false;
   }
   if(res < 200 || res >= 300) {
      if(g_dc.debug_log) Print("[DC] Health check rejected (status=", res, ")");
      return false;
   }
   string body = CharArrayToString(resp, 0, WHOLE_ARRAY, CP_UTF8);
...
void DC_Init(string url, string psk, int interval_sec, bool debug, datetime history_start_date) {
   ...
   g_dc.debug_log                 = debug;
   g_dc.last_sync_ms              = 0;
   g_dc.in_flight                 = false;
   g_dc.success_count             = 0;
   g_dc.error_count               = 0;
   g_dc.last_live_payload_hash    = 0;
   g_dc.last_live_payload_sent_ms = 0;
   g_dc.last_ack_history_hash     = "";
...
bool DC_Sync() {
   if(g_dc.url == "" || g_dc.psk == "") return false;
   ulong now_ms = GetTickCount64();
   if(g_dc.in_flight) {
      if(g_dc.debug_log && now_ms - g_dc.last_log_ms > 60000) {
         Print("[DC] Sync in flight, skipping");
         g_dc.last_log_ms = now_ms;
      }
      return false;
   }
   ...
   string closed_trades_json = DC_BuildClosedTradesJson(latest_closed_ms, latest_closed_id);
   ...
   if(payload_unchanged && since_ms < 60000) {
      if(g_dc.debug_log) Print("[DC] Payload unchanged, skipping sync");
      g_dc.last_sync_ms = now_ms;
      return false;
   }
   bool include_history = (history_hash != g_dc.last_ack_history_hash);
   string payload = StringFormat(
      "{\"account_number\":\"%s\",\"positions\":%s,\"closed_trades\":%s,\"account\":%s,"
      "\"sync_id\":\"%lld\",\"ea_latest_closed_time_ms\":%lld,\"ea_latest_closed_deal_id\":\"%s\","
      "\"open_positions_hash\":\"\",\"include_history\":%s,\"history_hash\":\"%s\"}",
      current_account, positions_json, closed_trades_json, account_json,
      timestamp_ms, latest_closed_ms, latest_closed_id,
      include_history ? "true" : "false", history_hash);
   if(g_dc.debug_log) Print("[DC] Payload: ", StringLen(payload), " bytes, include_history=", include_history ? "true" : "false");
   string signature  = DC_CreateSignature(current_account, timestamp_ms);
   g_dc.last_sync_ms = now_ms;
   g_dc.in_flight    = true;
   DC_PostSync(payload, current_account, signature, timestamp_ms, live_hash, include_history, history_hash);
   return true;
}
...
int   g_last_manual_tp_count     = 0;
int   g_last_partial_label_count  = 0;
color g_last_dot_color            = CLR_NONE;
bool  g_label_time_valid          = false;
datetime g_cached_label_time      = 0;

void UpdateStatusDot() {
   ulong now_ms = GetTickCount64();

   int success_count = g_dc.success_count;
   int error_count   = g_dc.error_count;
   ...
   if(success_count == 0 && error_count == 0)
      ...
      bool is_positive = g_dc.last_sync_ok;
...
input int    InpDashboardSyncIntervalSec = 1;        // Sync Interval (seconds)
...
// === PARTIAL STATE PERSISTENCE (file-based) ===
// Uses a binary file in MQL5/Files/ instead of MT5 GlobalVariables.
// This keeps the MT5 Globals list clean and is more reliable across restarts.

void SavePartialState() {
   if(!state.exists || !state.partialsValid) return;
   int h = FileOpen("STM17_" + _Symbol + ".bin", FILE_BIN | FILE_WRITE);
   if(h == INVALID_HANDLE) {
      Print("[STM] SavePartialState: cannot open file, error=", GetLastError());
      return;
   }
   FileWriteInteger(h, 1);  // version
   FileWriteInteger(h, state.partialCount);
   FileWriteInteger(h, state.partialMethod);
   FileWriteInteger(h, state.partialsValid ? 1 : 0);
   FileWriteDouble(h, state.partialFinalVolume);
   FileWriteDouble(h, state.lastPlannedPositionSize);
   FileWriteDouble(h, state.lastPlannedTP);
   for(int i = 0; i < 10; i++) FileWriteDouble(h, state.partialPrices[i]);
   for(int i = 0; i < 10; i++) FileWriteDouble(h, state.partialVolumes[i]);
   for(int i = 0; i < 10; i++) FileWriteInteger(h, state.partialExecuted[i] ? 1 : 0);
   for(int i = 0; i < 10; i++) FileWriteLong(h, (long)state.partialOrderTickets[i]);
   for(int i = 0; i < 10; i++) FileWriteLong(h, (long)state.partialPositionTickets[i]);
   FileWriteInteger(h, state.originalPartialCount);
   FileWriteDouble(h, state.originalPlannedPositionSize);
   FileWriteDouble(h, state.originalPartialFinalVolume);
   for(int i = 0; i < 10; i++) FileWriteDouble(h, state.originalPartialVolumes[i]);
   FileWriteInteger(h, state.beSet ? 1 : 0);
   FileWriteInteger(h, state.trailingActive ? 1 : 0);
   FileWriteDouble(h, state.trailingBestPrice);
   FileWriteDouble(h, state.userOverrideSL);
   FileWriteDouble(h, state.userOverrideTP);
   FileWriteInteger(h, state.executedPartialCount);
   for(int i = 0; i < 10; i++) FileWriteDouble(h, state.executedPartialPrices[i]);
   FileClose(h);
}
...
bool LoadPartialState() {
   string path = "STM17_" + _Symbol + ".bin";
   if(!FileIsExist(path)) return false;
   int h = FileOpen(path, FILE_BIN | FILE_READ);
   if(h == INVALID_HANDLE) return false;
   int ver = FileReadInteger(h);
   if(ver != 1) { FileClose(h); FileDelete(path); return false; }
   state.partialCount            = FileReadInteger(h);
   state.partialMethod           = FileReadInteger(h);
   state.partialsValid           = (FileReadInteger(h) != 0);
   state.partialFinalVolume      = FileReadDouble(h);
   state.lastPlannedPositionSize = FileReadDouble(h);
   state.lastPlannedTP           = FileReadDouble(h);
   for(int i = 0; i < 10; i++) state.partialPrices[i]  = FileReadDouble(h);
   for(int i = 0; i < 10; i++) state.partialVolumes[i] = FileReadDouble(h);
   for(int i = 0; i < 10; i++) state.partialExecuted[i] = (FileReadInteger(h) != 0);
   for(int i = 0; i < 10; i++) state.partialOrderTickets[i]   = (ulong)FileReadLong(h);
   for(int i = 0; i < 10; i++) state.partialPositionTickets[i] = (ulong)FileReadLong(h);
   state.originalPartialCount        = FileReadInteger(h);
   state.originalPlannedPositionSize = FileReadDouble(h);
   state.originalPartialFinalVolume  = FileReadDouble(h);
   for(int i = 0; i < 10; i++) state.originalPartialVolumes[i] = FileReadDouble(h);
   state.beSet            = (FileReadInteger(h) != 0);
   state.trailingActive   = (FileReadInteger(h) != 0);
   state.trailingBestPrice = FileReadDouble(h);
   state.userOverrideSL   = FileReadDouble(h);
   state.userOverrideTP   = FileReadDouble(h);
   state.executedPartialCount = FileReadInteger(h);
   for(int i = 0; i < 10; i++) state.executedPartialPrices[i] = FileReadDouble(h);
   FileClose(h);

   // Reconcile tickets: if a partial order was filled, find the resulting position
...
datetime GetLevelLabelTime() {
   if(g_label_time_valid) return g_cached_label_time;
   int visible_bars = (int)ChartGetInteger(0, CHART_VISIBLE_BARS);
   if(visible_bars <= 1) { g_cached_label_time = TimeCurrent(); g_label_time_valid = true; return g_cached_label_time; }
   ...
   g_cached_label_time = shifted_time;
   g_label_time_valid = true;
   return g_cached_label_time;
}
...
string FormatEntryLabel(bool is_buy, double total_volume, double entry_price) {
   return (is_buy ? "BUY " : "SELL ") + DoubleToString(total_volume, GetVolumeDisplayDecimals()) + "@" + DoubleToString(entry_price, sym.displayDecimals);
}
...
bool DrawPositionSingleTPGroups() {
   ...
   for(int g = 0; g < group_count && g < 10; g++) {
      ...
      string entry_label = group_tag + FormatEntryLabel(is_buy, group_total_vol, avg_entry);
...
void ClearBaseLevelLines() {
   ...
   for(int i = 0; i <= g_last_partial_label_count; i++) DeleteLineObjects("TM_Line_P" + IntegerToString(i));
   g_last_partial_label_count = 0;
   for(int i = 0; i <= g_last_manual_tp_count; i++) DeleteLineObjects("TM_Line_ManualTP" + IntegerToString(i));
   g_last_manual_tp_count = 0;
}
...
void UpdateLines() {
   ...
   ClearPendingGroupLines();
    ...
    // detailed single-TP groups for live entries.
   ...
   string entry_label = FormatEntryLabel(state.isBuy, vol, state.avgEntry);
   ...
   for(int i = manual_tp_label_idx; i <= g_last_manual_tp_count; i++) DeleteLineObjects("TM_Line_ManualTP" + IntegerToString(i));
   g_last_manual_tp_count = manual_tp_label_idx;
   ...
   for(int i = display_partial_count; i <= g_last_partial_label_count; i++) DeleteLineObjects("TM_Line_P" + IntegerToString(i));
   g_last_partial_label_count = display_partial_count;
