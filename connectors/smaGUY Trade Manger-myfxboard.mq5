...
//+------------------------------------------------------------------+
//| DashboardConnector - inlined (no external .mqh dependency)       |
//+------------------------------------------------------------------+
...
public:
   static void Init(string url, string psk, int interval_sec, int keepalive_sec = 30, bool debug = false) {
      ...
      s_sync_interval_ms = interval_sec * 1000;
         s_keepalive_ms     = MathMax(0, keepalive_sec) * 1000; // Keepalive interval in milliseconds
      s_debug_log        = debug;
      s_last_sync_ms     = 0;
      s_in_flight        = false;
      s_success_count    = 0;
...
input int    InpDashboardKeepaliveSec = 30;          // Keepalive Interval (seconds, 0=disable)
