    ...
    try {
      ...
      updateAccountCache(
        ...
        positionsWithMargin.map((p: any): CachedPosition => ({
          ...
          symbol: p.symbol,
          size: p.volume,
          size_units: Number.isFinite(Number(p.size_units)) ? Number(p.size_units) : null,
          contract_size: Number.isFinite(Number(p.contract_size)) ? Number(p.contract_size) : null,
        ...
        if (shouldIncludeHistory) {
          for (const trade of closed_trades) {
            const tradeRecord: Omit<Trade, 'id'> = {
              ...
              symbol: trade.symbol,
              size: trade.volume,
              size_units: Number.isFinite(Number(trade.size_units)) ? Number(trade.size_units) : null,
              contract_size: Number.isFinite(Number(trade.contract_size)) ? Number(trade.contract_size) : null,
